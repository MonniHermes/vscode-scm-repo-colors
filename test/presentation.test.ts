...
const summary = (name: string, dirty: boolean): RepositorySummary => ({
  name,
  branch: 'main',
  status: aggregateStatus({ indexChanges: dirty ? [{ status: 5 }] : [], workingTreeChanges: [], mergeChanges: [] })
});
...
test('formats deterministic compact and complete summaries', () => {
  const item: RepositorySummary = {
    ...
    status: aggregateStatus({ indexChanges: [{ status: 1 }], workingTreeChanges: [{ status: 5 }], mergeChanges: [], ahead: 2 })
  };
